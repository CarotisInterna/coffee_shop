...
public class AppUserController {
    ...
    @PostMapping
    public ResponseEntity<AppUserDto> register(@Valid @ModelAttribute("appUser") NewAppUserDto newAppUserDto, BindingResult result) {
        if (result.hasErrors()) {
            throw new ValidationException(result);
        }
        return ResponseEntity.ok(appUserService.saveAppUser(newAppUserDto));
    }
}
